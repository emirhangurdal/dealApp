                           ...
                           let date = data["Date"] as? Double {
                            if ProfileDeals.shared.blockedUsersIDs.contains(where: {$0.id == senderUID}) == false {
                                if Date().timeIntervalSince1970 - date < 86400 {
                                    self.getImageFromURLfromFirebase(path: image, dealTitle: dealTitle, dealDesc: dealDesc, storeTitle: storeTitle, storeID: storeID, dealID: dealID, sender: sender, userName: userName, distance: distance, senderUID: senderUID, countDown: Int(Date().timeIntervalSince1970 - date))
                                }
...
//            storeImage.bottom.equalTo(detailView).offset(-50)
//            storeImage.left.equalTo(detailView)
//            storeImage.right.equalTo(detailView)
//        }
