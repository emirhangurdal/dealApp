...
class DealsContentVC: UIViewController, UIScrollViewDelegate, UITableViewDelegate, StoresFeedDelegate2, CreateAlert, PushProfilePage, BlockUser {
    ...
    private var submitCoupon: UIButton = {
        var bttn = UIButton()
        bttn.setTitle("Submit Deal".localized(), for: .normal)
        ...
        bttn.addTarget(self, action: #selector(submitCoupons), for: .touchUpInside)
        return bttn
    }()
    @objc func submitCoupons() {
        let postDeal = PostDealVC()
        self.present(postDeal, animated: true)
    }
   
    //MARK: Show Local Deals
    func showLocalDeals() {
    ...
    func configureConstraints(){
        ...
        tableView.snp.makeConstraints { tableView in
            ...
            tableView.bottom.equalTo(view.safeAreaLayoutGuide).offset(-50)
        }
        
        submitCoupon.snp.makeConstraints { submitCoupon in
            submitCoupon.width.equalTo(view.safeAreaLayoutGuide).multipliedBy(0.90)
            submitCoupon.height.equalTo(35)
            submitCoupon.centerX.equalTo(view.safeAreaLayoutGuide)
            submitCoupon.top.equalTo(tableView.snp.bottom).offset(1)
        }
        configureNavBar()
    }
    ...
    func configureNavBar(){
        ...
        self.navigationItem.title = "Local Deals".localized()
...
//                           let image = data["ImagePath"] as? String,
                           ...
                           let date = data["Date"] as? Double {
                            ...
                            if ProfileDeals.shared.blockedUsersIDs.contains(where: {$0.id == senderUID}) == false {
                                if Date().timeIntervalSince1970 - date < 86400 {
                                    self.getDealData(dealTitle: dealTitle, dealDesc: dealDesc, storeTitle: storeTitle, storeID: storeID, section: index, dealID: dealID, sender: sender, userName: userName, distance: distance, senderUID: senderUID, date: date, countDown: difference)
                                }
                            }
    ...
    //MARK: - Get Weekly Ads Link from FireBase
    ...
    func getDealData(dealTitle: String, dealDesc: String, storeTitle: String, storeID: String, section: Int, dealID: String, sender: String, userName: String, distance: Double, senderUID: String, date: Double, countDown: Int) {
        DealsData.shared.dealsArray[section].date = date
        DealsData.shared.dealsArray[section].items.append(DealModel(storeID: storeID, dealImage: UIImage(named: "user-deal") ?? UIImage(named: "no-image"), dealTitle: dealTitle, dealDesc: dealDesc, dealID: dealID, storeTitle: storeTitle, sender: sender, userName: userName, distance: distance, senderUID: senderUID, countDown: countDown))
